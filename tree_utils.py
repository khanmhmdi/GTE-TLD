...
class Tree:
    ...
    def evaluate_tree(self, variable_values):
        """Evaluates the mathematical expression represented by the tree.

        Args:
            variable_values (dict): A dictionary mapping variable names to their numerical values.

        Returns:
            float: The evaluated result of the expression.
            None: If an error occurs during evaluation.

        Raises:
            ValueError: If an unsupported operation is encountered.
        """

        if isinstance(self.value, float):  # Base case: numerical value
            return self.value

        elif isinstance(self.value, str):  # Variable
            if self.value in variable_values:
                return variable_values[self.value]
            else:
                # Handle missing variable
                print(f"Warning: Variable '{self.value}' not found in provided values.")
                return None

        else:  # Operation
            left_result = self.children[0].evaluate_tree(variable_values)
            right_result = self.children[1].evaluate_tree(variable_values)

            if left_result is None or right_result is None:
                return None  # Propagate error if child evaluation fails
            ...
            if self.value == '+':
                ...
                if not isinstance(left_result, float):
                    raise ValueError(f"Invalid argument type for '{self.value}': {left_result}")
                if self.value == 'sin':
                    return np.sin(left_result)
                elif self.value == 'cos':
                    return np.cos(left_result)
                elif self.value == 'tan':
                    return np.tan(left_result)
                elif self.value == 'exp':
                    return np.exp(left_result)
                elif self.value == 'log':
                    return np.log(left_result)
                elif self.value == 'sqrt':
                    return np.sqrt(left_result)
            else:
...
class Vocab:
    def __init__(
        ...
        self.non_terminal_add_token = "<NON-TERMINAL-ADD>"
        self.non_terminal_mul_token = "<NON-TERMINAL-MUL>"
        self.non_terminal_div_token = "<NON-TERMINAL-DIV>"
        self.non_terminal_pow_token = "<NON-TERMINAL-POW>"

        self.non_terminal_unary_token = "<NON-TERMINAL-UNARY>"
        self.non_terminal_number_token = "<NON-TERMINAL-NUMBER>"
        ...
        self.add_symbol(self.non_terminal_add_token)
        self.add_symbol(self.non_terminal_mul_token)
        self.add_symbol(self.non_terminal_div_token)
        self.add_symbol(self.non_terminal_pow_token)

        self.add_symbol(self.non_terminal_unary_token)
        self.add_symbol(self.non_terminal_number_token)
