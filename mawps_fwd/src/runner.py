...
class Mawps:
    ...
    def _build_model(self):
        ...
        self.model = Graph2Tree.load_checkpoint(self.opt["checkpoint_save_path"], "epoch_170.pt").to(
            self.device
        )
    ...
    def _build_optimizer(self):
        ...
        # self.optimizer = self.model["optimizer"](parameters, lr=self.opt["learning_rate"], weight_decay=self.opt["weight_decay"])
    ...
    def train(self):
        ...
        for epoch in range(1, self.opt["max_epochs"] + 1):
            ...
            for name, param in self.model.named_parameters():
                writer.add_histogram(name, param, epoch)  # Log weight histograms
            end_time = time.time()  # Record epoch end time
            train_time = end_time - start_time

            logging.info("Epoch: {}, Train Loss: {:.3f}, Train Time: {:.2f}s".format(epoch, loss_to_print, train_time))
            writer.add_scalar("Train_Loss", loss_to_print, epoch)  # Log training loss

            # if epoch % 5 == 0 or epoch == 1:
            self.model.save_checkpoint(self.opt["checkpoint_save_path"], f"epoch_{epoch}.pt")

            # if epoch % 5 == 0:
            #     self.model.save_checkpoint(self.opt["checkpoint_save_path"], f"epoch_{epoch}.pt")
            #     if loss_to_print < last_loss:
            #         last_loss = loss_to_print
    ...
    def eval(self, model, mode="val"):
        ...
        with open("reference_list.txt", "w") as f:
            f.write(str(reference_list) + "\n")

        with open("candidate_list.txt", "w") as f:
            f.write(str(candidate_list) + "\n")
        print("Lists saved to lists.txt!")
        print('hi')
        # with open("reference_list.txt", "r") as f:
        #     reference_list = f.read().strip().strip("[").strip("]").split(", ")
        #
        # # Read candidate list from file
        # with open("candidate_list.txt", "r") as f:
        #     candidate_list = f.read().strip().strip("[").strip("]").split(", ")
...
if __name__ == "__main__":
    ...
    best_acc = runner.eval(runner.model)
    end = time.time()
